...
export class SkillManager {
  ...
  private loadBuiltinSkills(): void {
    ...
    this.skills.set("send-email", {
      name: "send-email",
      description: "Send emails via Resend API with HTML content and file attachments",
      triggers: [
        "send email", "email", "send mail", "notify via email", "email report",
        "send notification email", "email attachment", "send attachment",
        "mail to", "send to email",
      ],
      instructions: [
        `When asked to send an email:`,
        ``,
        `1. Use the send_email tool. Configuration is automatic via RESEND_API_KEY and RESEND_FROM_EMAIL env vars.`,
        ``,
        `2. Required parameters:`,
        `   - to: recipient email(s) — string or array`,
        `   - subject: email subject line`,
        `   - html or text: email body (prefer html for rich formatting)`,
        ``,
        `3. Optional parameters:`,
        `   - from: override sender (default: RESEND_FROM_EMAIL)`,
        `   - cc: CC recipients`,
        `   - bcc: BCC recipients`,
        `   - reply_to: reply-to address(es)`,
        `   - attachments: array of { path: '/absolute/path/to/file', filename: 'optional-name.pdf' }`,
        ``,
        `4. For HTML emails, write clean, inline-styled HTML. Email clients have limited CSS support`,
        `   — use inline styles, tables for layout, and avoid external stylesheets.`,
        ``,
        `5. For attachments: use absolute file paths. The tool reads and base64-encodes files automatically.`,
        ``,
        `6. Example:`,
        `   send_email({`,
        `     to: 'user@example.com',`,
        ...
        `   })`,
        ``,
        `7. If not configured, tell the user to set RESEND_API_KEY and RESEND_FROM_EMAIL in ~/.beercan/.env`,
      ].join("\n"),
